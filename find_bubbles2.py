...
def find_sb_alg(graph, s, direction, chain):
	"""
	takes the graph and a start node s and add a bubble to the chain if one is found
	if s was the source
	"""
	...
	while len(S) > 0:
		...
		if (len(S) == 1) and (len(seen) == 1):
			...
			bubble = Graph2.Bubble(source=s, sink=t[0], inside=nodes_inside)
